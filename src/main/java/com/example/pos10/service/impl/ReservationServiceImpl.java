...
//
//  	    if (hours.isEmpty()) {
//  	        return new ReservationRes(400, "當天無營業時間！");
//  	    }
//
...
//  	        Integer cleaningBreak = (Integer) hour[3]; // 可以為 null
//
//  	        // 計算可用時間段
...
//
//  	            // 為當前時間段創建查詢請求
//  	            ReservationAndTableReq req = new ReservationAndTableReq();
//  	            req.setReservationDate(reservationDate);
//  	            req.setReservationPeople(reservationPeople);
//  	            req.setReservationStartTime(slot);
//  	            req.setReservationEndTime(nextSlot);
//
//  	            // 查詢該時間段內是否有可用桌位
//  	            List<TableManagement> availableTablesForTimeSlot = reservationAndTableService.getAvailableTables(req);
//
//  	            // 如果有可用桌位，將時間段標記為可用
//  	            boolean isAvailable = !availableTablesForTimeSlot.isEmpty();
//  	            availableTimeSlots.add(new AvailableTimeSlot(slot.toString(), nextSlot.toString(), isAvailable));
//  	        }
//  	    }
//
...
//  	    }
//
//  	    // 2. 驗證電話號碼格式（09開頭，後面接8個數字）
//  	    String phoneNumber = reservationReq.getCustomerPhoneNumber();
//  	    if (phoneNumber == null || !phoneNumber.matches("^09\\d{8}$")) {
...
//  	    // 6. 自動分配桌位
//  	    ReservationAndTableReq req = new ReservationAndTableReq();
//  	    req.setReservationDate(reservationReq.getReservationDate());
//  	    req.setReservationStartTime(reservationReq.getReservationStartTime());
//  	    req.setReservationEndTime(reservationReq.getReservationEndingTime());
//  	    req.setReservationPeople(reservationReq.getReservationPeople());
//
//  	    ReservationAndTableRes tableAssignRes = reservationAndTableService.autoAssignTable(req);
//
//  	    if (tableAssignRes.getCode() != 200) {
//  	        return new ReservationRes(tableAssignRes.getCode(), tableAssignRes.getMessage());
//  	    }
//
//  	    // 7. 儲存訂位資訊 (先儲存 Reservation)
...
//  	    reservation.setReservationDate(reservationReq.getReservationDate());
//  	    reservation.setReservationStartTime(reservationReq.getReservationStartTime());
//  	    reservation.setReservationEndingTime(reservationReq.getReservationEndingTime());
//
//  	    // 先儲存 Reservation，獲取 reservationId
//  	    reservationDao.save(reservation);
//
//  	    // 8. 儲存選擇的桌位到 ReservationAndTable
//  	    List<String> tableNumbers = tableAssignRes.getTableNumbers();
//  	    List<TableManagement> selectedTables = tableManagementDao.findByTableNumberIn(tableNumbers);
//  	    
//  	    List<ReservationAndTable> reservationAndTableList = new ArrayList<>();
//  	    
//  	    for (TableManagement table : selectedTables) {
//  	        // 創建 ReservationAndTable 複合鍵
//  	        ReservationAndTableId reservationAndTableId = new ReservationAndTableId();
//  	        reservationAndTableId.setReservationId(reservation.getReservationId());
//  	        reservationAndTableId.setTableNumber(table.getTableNumber());
//  	        
//  	        // 建立 ReservationAndTable 實體
//  	        ReservationAndTable reservationAndTable = new ReservationAndTable();
//  	        reservationAndTable.setReservationTableId(reservationAndTableId);  // 設置複合鍵
//  	        reservationAndTable.setReservation(reservation);  // 設置與該預約的關聯
//  	        reservationAndTable.setTable(table);              // 設置與該桌位的關聯
//  	        reservationAndTable.setReservationDate(reservation.getReservationDate()); // 設定 reservationDate
//  	        reservationAndTable.setReservationStartTime(reservation.getReservationStartTime());
//  	        reservationAndTable.setReservationEndTime(reservation.getReservationEndingTime());
//
//  	        // 設定桌位狀態為 "訂位中"
//  	        reservationAndTable.setTableStatus(TableStatus.訂位中);
//
//  	        reservationAndTableList.add(reservationAndTable);
//  	    }
//  	    
//  	    // 9. 將這些 ReservationAndTable 儲存到資料庫
//  	    reservationAndTableDao.saveAll(reservationAndTableList);
//
//  	    // 發送訂位確認信
//  	    try {
